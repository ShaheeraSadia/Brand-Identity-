...
5. Scores out of 100 for these 6 major archetypes for the radar chart: 'The Creator', 'The Hero', 'The Sage', 'The Magician', 'The Explorer', 'The Ruler'. Ensure the primary archetype matches one of these or is highly related, and has the highest score.`;
      ...
      console.warn("Error generating brand archetype, returning calculated fallback:", error?.message || error);
      const archetypes = ["The Creator", "The Hero", "The Sage", "The Magician", "The Explorer", "The Ruler"];
      const primary = archetypes[Math.floor(Math.random() * archetypes.length)];
      res.json({
        primaryArchetype: primary,
        tagline: `${req.body?.companyName || "Brand"} - Powered by Purpose and Excellence`,
        summary: `${req.body?.companyName || "The brand"} aligns with ${primary}, uniting visionary ambition with clear execution for ${req.body?.targetAudience || "its core audience"}.`,
        attributes: ["Visionary", "Authentic", "Impactful"],
        scores: [
          { archetype: "The Creator", score: primary === "The Creator" ? 95 : 75, description: "Inspiring innovation and creativity" },
          { archetype: "The Hero", score: primary === "The Hero" ? 95 : 70, description: "Mastering challenges and driving results" },
          { archetype: "The Sage", score: primary === "The Sage" ? 95 : 80, description: "Clarity, wisdom, and data-driven insight" },
          { archetype: "The Magician", score: primary === "The Magician" ? 95 : 68, description: "Transforming experiences into moments of delight" },
          { archetype: "The Explorer", score: primary === "The Explorer" ? 95 : 72, description: "Pioneering new approaches with courage" },
          { archetype: "The Ruler", score: primary === "The Ruler" ? 95 : 65, description: "Commanding standard-setting authority" }
        ]
      });
    }
  });
...
3. The raw, valid SVG markup string (svgMarkup).`;
      ...
      console.warn("Error generating brand pattern, using fallback:", error?.message || error);
      const name = req.body?.companyName || "Brand";
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 60 60">
  <defs>
    <pattern id="grid" width="30" height="30" patternUnits="userSpaceOnUse">
      <path d="M 30 0 L 0 0 0 30" fill="none" stroke="#4F46E5" stroke-width="0.75" stroke-opacity="0.3"/>
      <circle cx="30" cy="0" r="1.5" fill="#7C3AED" fill-opacity="0.4"/>
    </pattern>
  </defs>
  ...
  <rect width="100%" height="100%" fill="url(#grid)"/>
      ...
      res.json({
        patternName: `${name} Architectural Matrix`,
        description: `A precision geometric grid pattern with rhythmic node intersections, reflecting balance, technical structure, and forward momentum.`,
        svgMarkup: svg
      });
    }
  });
...
Generate a new, perfectly matched 5-color palette. Each color MUST have a hex code, a creative and evocative color name, a specific role (one of: 'Primary', 'Secondary', 'Accent', 'Dark Neutral', 'Light Neutral'), and a detailed usage direction. Make sure the 5 roles are distinct (having one of each role is ideal, or well balanced). Return the new palette matching the schema.`;
      ...
      console.warn("Error shuffling palette, using calculated variation:", error?.message || error);
      const fallbackPalette = generateFallbackPaletteVariation(req.body, req.body?.shuffleType);
      res.json({ colorPalette: fallbackPalette });
    }
  });
...
Also provide a 'rationale' (2-3 sentences) explaining how this fresh 5-color palette expresses the brand's core mission and emotional identity.`;
      ...
      console.warn("Error regenerating palette from mission, using calculated variation:", error?.message || error);
      const fallbackPalette = generateFallbackPaletteVariation(req.body, req.body?.customFocus);
      res.json({
        colorPalette: fallbackPalette,
        rationale: `Harmonious 5-color system calibrated to balance primary energy with refined dark neutrals and clean light accents.`
      });
    }
  });
...
Please analyze the brand and any provided logo image to create this favicon.`;
      ...
      console.warn("Error generating brand favicon, using fallback vector:", error?.message || error);
      const companyName = req.body?.companyName || "Brand";
      const initial = companyName.trim().charAt(0).toUpperCase() || "B";
      const primaryHex = req.body?.colorPalette?.[0]?.hex || "#4F46E5";
      const secondaryHex = req.body?.colorPalette?.[1]?.hex || "#7C3AED";
  ...
  <text x="50%" y="56%" dominant-baseline="middle" text-anchor="middle" font-family="system-ui, -apple-system, BlinkMacSystemFont, sans-serif" font-weight="900" font-size="34" fill="#FFFFFF">${initial}</text>
</svg>`;
      res.json({
        faviconName: `${companyName} Monogram Badge`,
        explanation: `Precision vector monogram glyph on squircle substrate, optically weighted for clarity at 16x16 and 32x32 browser tab resolutions.`,
        svgMarkup: svg
      });
    }
  });
...
Return a JSON object matching the schema.`;
      ...
      console.warn("Error generating voice preview, using fallback:", error?.message || error);
      const company = req.body?.companyName || "Brand";
      const mission = req.body?.mission || "Exceptional standards in design and execution.";
      res.json({
        headline: `Engineered for Purpose. Built for ${company}.`,
        paragraph: `At ${company}, we believe remarkable outcomes begin with unapologetic clarity. Rooted in our commitment to ${mission.toLowerCase().replace(/\.$/, '')}, we craft every touchpoint with deliberate intent and enduring quality.`,
        toneAlignmentNote: `Calibrated to reflect modern brand clarity with high audience resonance.`
      });
    }
  });
...
Make sure all hex codes start with '#' and contain valid 6-character hex colors. Ensure roles are distinct.`;
      ...
      console.warn("Chat error, returning consultative fallback response:", error?.message || error);
      res.json({
        text: `As your Brand Strategy Consultant, I've reviewed the strategic positioning for ${req.body?.brandBible?.companyName || "your brand"}. To strengthen market differentiation, maintain optical consistency across your design system and ensure your verbal identity remains faithful to your core mission. How can I assist you with refining your color hierarchy, voice guidelines, or competitive positioning?`
      });
    }
  });
